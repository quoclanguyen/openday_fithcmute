...
def a_star(GUI, Grid, start, end):
    ...
    g_score[flat(start, Grid.size)] = 0
    f_score = [float("inf") for row in Grid.data for _ in row]
    f_score[flat(start, Grid.size)] = heuristics(start, end)
    ...
    while not states.empty():
        ...
        for nei in Grid.neighbors[flat(current, Grid.size)]:
            temp_g = g_score[flat(current, Grid.size)] + 1
            if temp_g >= g_score[flat(nei, Grid.size)]:
                continue
            came_from[nei] = current
            g_score[flat(nei, Grid.size)] = temp_g
            f_score[flat(nei, Grid.size)] = temp_g + heuristics(nei, end)   # f = g + h
            if nei in states_history:
                continue
            count += 1
            states.put((f_score[flat(nei, Grid.size)], count, nei))
...
def ucs(GUI, Grid, start, end):
    ...
    while not states.empty():
        ...
        if current == end:
            return came_from, count
        for nei in Grid.neighbors[flat(current, Grid.size)]: # grid around current point
            temp_g_score = g_score[flat(current, Grid.size)] + 1
            if temp_g_score < g_score[flat(nei, Grid.size)]:
                came_from[nei] = current
                g_score[flat(nei, Grid.size)] = temp_g_score
                if nei in states_history:
                    continue
                count += 1
                states.put((g_score[flat(nei, Grid.size)], count, nei))
                states_history.add(nei)
                if nei != end:
                    simulate(GUI, nei)
    return 0, 0
...
def dfs(GUI, Grid, start, end):
    ...
    while len(states) != 0:
        ...
        if current == end:
            came_from.pop(start)
            return came_from, count
        neis = Grid.neighbors[flat(current, Grid.size)]
...
def greedy(GUI, Grid, start, end):
    ...
    while not states.empty():
        ...
        if current == end:
            ...
            return came_from, count
        for nei in Grid.neighbors[flat(current, Grid.size)]: # grid around current point
            f_score[flat(nei, Grid.size)] = heuristics(nei, end) 
            if nei in states_history:
                continue
            came_from[nei] = current
            count += 1
...
def beam(GUI, Grid, start, end):
    ...
    while not states.empty():
        ...
        if current_second == end:
            came_from.pop(start, None)
            return came_from, count
        for nei in Grid.neighbors[flat(current_first, Grid.size)]:
            f_score[flat(nei, Grid.size)] = heuristics(nei, end)
            if nei in states_history:
                continue
            ...
            if nei != end:
                simulate(GUI, nei)
        for nei in Grid.neighbors[flat(current_second, Grid.size)]:
            f_score[flat(nei, Grid.size)] = heuristics(nei, end)
            if nei in states_history:
                continue
