...
class HomeGUI:
    ...
    def clearWarning(self):
        warnRect = [
            0,
            self.buttons["AI"].y + 100,
            self.width * 2, 100
        ]
        self.screen.fill(colorsData.bgColor, warnRect)
    ...
    def startButton(self):
        if self.ai == False and self.human == False:
            self.warning("You must choose a gamemode!")
            return
        mode = "ai" if self.ai else "human"
        MazeApp(self.size, self.width, self.height, mode, self.bg_music)
        self.bg_music.play()
        self.drawBackground()
    def exitButton(self):
        self.done = True
    ...
    def drawing(self):
        ...
        self.buttons["Logo"] = Button(
            (self.width - 770)//2, 
            self.buttons["Size"].y + 140,
            "./images/logo.png",
            1
        )
        ...
        self.buttons["Logo"].draw(self.screen, lambda:0)
